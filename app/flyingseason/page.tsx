...
export default function FlyingSeasonPage() {
  ...
  return (
    <main className="min-h-screen bg-black text-white flex flex-col items-center justify-start py-10 px-4">
      ...
      <div className="flex flex-col items-center justify-center w-full px-4 mt-24">
        <div className="w-full max-w-[800px] border-[4px] border-black/50 rounded-2xl p-4 md:p-8 z-10 bg-black/30 shadow-lg flex flex-col items-center">
          <h1 className="text-2xl md:text-4xl font-bold mb-6 text-center z-10">
            Migration – {selectedBird === 'hummingbird' ? 'Rufous Hummingbird' : selectedBird === 'eagle' ? 'Bald Eagle' : 'Harlequin Duck'}
          </h1>
          ...
          <div className="w-full max-w-[800px] border-[4px] border-black/50 rounded-2xl p-4 mt-5 z-10 bg-black/50 shadow-lg flex flex-col items-center">
            <div className="text-center mt-2 mb-8 z-10">
              <h2 className="text-3xl font-bold text-white">Non-Migrating Birds</h2>
              <p className="text-sm text-white/70">Click a card to flip and learn more</p>
            </div>
            <section className="grid grid-cols-2 gap-8 py-16 px-4 bg-green-950 max-w-[600px] rounded-xl z-10 shadow-md">
                ...
                <div key={index} className="w-[230px] h-[380px] perspective" onClick={() => toggleCard(index)}>
                  <div className={`relative w-full h-full transition-transform duration-700 preserve-3d ${flippedIndex === index ? 'rotate-y-180' : ''}`}>
                    ...
                    <div className={`absolute w-full h-full backface-hidden rotate-y-180 ${bird.bgColor} border-[3px] border-yellow-500 rounded-[10px] p-3 flex flex-col justify-between z-0`} style={{ fontFamily: 'Arial, sans-serif' }}>
                      <div className="text-[18px] font-bold leading-none flex justify-between items-center px-[2px] text-black">
                        <span>{bird.name}</span>
                        <span>{bird.icon}</span>
                      </div>
                      ...
                      <div className="text-[13px] text-gray-800 bg-white/40 border-[1px] border-black p-2 mb-1 shadow-sm rounded-sm">
                        {bird.description}
                      </div>
            ...
            <div className="flex justify-center mt-6 mb-2">
              <motion.button
                ...
                className="bg-green-700 hover:bg-green-600 text-white font-semibold px-6 py-2 rounded shadow-md transition"
